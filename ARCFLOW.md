...
| `lib/marketData/liquidityFilter.ts` | Filtro de liquidez ($50K vol 24h, 50% range) |
| `lib/marketData/marketHours.ts` | isUSStockMarketOpen() + helpers (NYSE 9:30-16:00 ET) |
| `lib/marketData/BackpackScanner.ts` | Descoberta dinâmica de símbolos, filtro + market hours, ranking por score dos agentes |
...
## 52. SESSION SUMMARY (02/07/2026 noite) — quoteVolume fix: NASDAQ reference data sem magic numbers
...
### RI-BANK-73/74 — a causa real das falhas do RI-BANK-71 não era instabilidade de RPC, era ABI incorreta

**O que o RI-BANK-71 realmente diagnosticou errado:** depois do RI-BANK-72 ir para produção, a Ordem 1/3 falhou mais 3 vezes seguidas, agora com a mensagem honesta `verification_failed` (não mais mascarada) — mas *sempre*, mesmo com resiliência de rede validada. RI-BANK-73 investigou isso lendo o RPC diretamente, fora do código do projeto: os 4 endpoints (primário + 3 backups) respondiam rápido (sub-segundo) e de forma consistente, todos com o mesmo `execution reverted` sem dado de revert — não é o padrão de uma rede instável, é o padrão de um contrato recusando uma chamada específica de forma determinística.

**Causa raiz confirmada via `eth_getCode`:** o pool USDC/EURC (`0xA1e418D16C969FdB9482716C7e2bD3d31872EBfb`) não é um par Uniswap V2 padrão — é `contracts/GenericAMMPair.sol`, um AMM customizado. Suas reservas são duas variáveis públicas separadas, `reserve0`/`reserve1` (`uint256` cada, getters auto-gerados `0x443cb4bc`/`0x5a76f25e`), **não** um `getReserves()` combinado (`0x0902f1ac`, o seletor Uniswap V2 padrão que `readReserves()` vinha chamando desde RI-BANK-70). Esse seletor nunca existiu nesse contrato — toda chamada com ele cai no fallback padrão do Solidity, que reverte sem dado. Os getters reais, testados diretamente via `eth_call`, devolveram os valores corretos e estáveis: 17,78 USDC / 15,551386 EURC — a liquidez sempre esteve lá, intacta.

Isso muito provavelmente também explica a *primeira* falha do RI-BANK-71 — na época atribuída a flakiness de RPC, porque o `ethers` reporta um blip de rede transitório e um revert determinístico de contrato com a mesma mensagem (`CALL_EXCEPTION: "missing revert data"`), tornando os dois indistinguíveis sem inspecionar a chamada RPC crua.

**Correção (`lib/route-verifier.ts`):** `readReserves()` agora chama `reserve0()`/`reserve1()` (getters reais do `GenericAMMPair`) em vez de `getReserves()`. `checkRouteViaMulticall()` (usado por `hasSellRoute()` no pipeline mainnet do Pregão, RI-BANK-67/69) tinha exatamente o mesmo bug — mas como usa `allowFailure: true` no Multicall3, o revert nunca aparecia como erro: simplesmente virava `success: false` silenciosamente, e a função reportava "sem rota" sem nunca logar nada. Corrigido para o mesmo par de getters, com duas chamadas por pool no lote do multicall em vez de uma. Confirmado que a correção também funciona para o outro pool conhecido (USDC/cirBTC, `contracts/MicroPool.sol`) — esse contrato tem `reserve0`/`reserve1` como variáveis públicas (mesmos seletores) *e* um `getReserves()` próprio; testado ao vivo via `eth_call` e os valores batem exatamente com os já observados (1,00 USDC / 0,0001 cirBTC).
...
### RI-BANK-80 — a execução não realimentava o aprendizado do Bandit (diagnóstico)

**Achado:** a primeira execução real decidida pelo Bandit (RI-BANK-79) teve sucesso on-chain, mas `bandit:state` continuou com `trades: 0`/`profit: 0` — nenhum elo conectava o resultado de volta ao estado. Confirmado no código: `recordBanditResult()` nunca era chamado por `lib/cron-trading-service.ts`/`lib/cron-trading-runtime.ts`. Não era uma decisão de design deliberada — nenhuma das seções RI-BANK-76/78/79 documentava esse fechamento de ciclo como "deixado para depois"; a lacuna passou despercebida em todas as três etapas. Achado adicional: mesmo com o elo construído, `CronExecutionResult` não carregava nenhum dado de lucro/custo — só sucesso/falha — então faltava também o próprio cálculo, não só a chamada.

### RI-BANK-81 — lucro real (câmbio externo), conectado ao aprendizado do Bandit

**Fonte de câmbio externa real (`lib/external-forex-rate.ts`):** `fetchExternalUsdEurRate()` consulta a Frankfurter API (dados de referência do Banco Central Europeu, sem chave necessária) — uma API de câmbio fiat dedicada, deliberadamente escolhida em vez de reaproveitar a cotação de EURC já usada em outro lugar do sistema (`lib/sosovalue-price-agent.ts`, um agregador de preço de ativo cripto, não uma fonte de câmbio fiat). Mesmo padrão de resiliência já validado (RI-BANK-50/62/72): `withRetries()` absorve um blip transitório, mas **nunca fabrica uma taxa** — se todas as tentativas falharem, propaga o erro em vez de cair num fallback 1:1 silencioso, que reintroduziria exatamente a distorção que este ticket existe para evitar.

**Cálculo de lucro (`lib/bandit-execution-feedback.ts`):** `computeSwapProfitUsd()` converte as quantidades reais de entrada/saída do swap (já apuradas por `real-swap-executor.ts` via saldo on-chain antes/depois — `postSwapBalance - preSwapBalance`, não uma estimativa) para USD usando a taxa externa, nunca o preço interno do pool. Restrito deliberadamente a USDC/EURC hoje — os únicos dois lados com fonte de câmbio fiat direta conhecida; um par envolvendo cirBTC (sem fonte de câmbio fiat, precisaria de uma fonte de preço de ativo — categoria de problema dos futuros robôs garimpeiros, fora de escopo) lança erro em vez de assumir 1:1 silenciosamente.

**Marcador de proveniência (`environment`, `lib/bandit-state-redis.ts`):** novo campo global no estado persistido, gravado uma vez via `HSETNX` (nunca sobrescrito), valor `"testnet"` — hardcoded pelo mesmo motivo de isolamento do resto do módulo (hoje só existe um ambiente real em uso). Documenta explicitamente que, mesmo usando câmbio externo real, a **liquidez** continua sendo de testnet — os pesos aprendidos aqui não devem ser presumidos diretamente aplicáveis a mainnet sem revisão. "Preço confiável" (câmbio externo, RI-BANK-81) e "liquidez ainda não representativa" (testnet, RI-BANK-70/73/77) são duas questões distintas — resolver a primeira não resolve a segunda.

**Integração (`CronExecutionResult` → `recordBanditResult()`):** `executeCronPlanWithKms()` (`lib/cron-trading-runtime.ts`) calcula `banditProfitUsd`/`banditPriceSource`/`banditEnvironment` só quando `plan.strategy === "bandit-decision"` e a execução foi real — nunca para planos manuais, nunca quando o câmbio externo falha (o campo fica `undefined`, não zero). `lib/cron-trading-service.ts`, logo após persistir `status: "completed"` e o evento de auditoria, chama a nova dependência opcional `recordBanditResult(pairLabel, profitUsd)` **só** quando `executed && claimed.strategy === "bandit-decision" && execution.banditProfitUsd !== undefined` — condicionado exatamente como o RI-BANK-80 havia identificado. Uma falha nessa chamada nunca desfaz a execução já liquidada on-chain (só logada — o trade já aconteceu; apenas o aprendizado do Bandit fica pendente até a próxima vez).

**Teste de regressão:**
- `lib/security/ri-bank-81-external-forex-profit.test.ts` — USDC 1:1 com USD, EURC usa a taxa externa injetada (não 1:1), par não suportado (cirBTC) falha fechado, e `fetchExternalUsdEurRate()` de fato tenta de novo numa falha transitória mockando `fetch` global antes de desistir (confirmado empiricamente em RI-BANK-72 que `fetch()` direto — diferente de `ethers.JsonRpcProvider` — responde a esse mock) e nunca fabrica uma taxa quando todas as tentativas falham.
- `lib/security/ri-bank-81-bandit-feedback-cycle.test.ts` — ciclo completo com `CronTradingService` inteiramente mockado (mesmo padrão de `ri-bank-34-cron-trading.test.ts`): uma execução `bandit-decision` bem-sucedida atualiza `bandit:state` de verdade (via Redis falso em memória, `recordBanditResult()` real); um plano `manual-validated`, mesmo com sucesso e mesmo que `banditProfitUsd` estivesse presente por engano, nunca aciona a realimentação; e a ausência de lucro calculado (câmbio externo indisponível) nunca gera uma chamada com valor fabricado.

### RI-BANK-82/83 — mismatch de caixa bloqueava `cirBTC` no plano do Bandit

**Achado (RI-BANK-82):** a primeira decisão real do Bandit em `cirBTC→USDC` falhou com `cron_plan_token_not_configured` antes de qualquer transação. Causa: `ri-bank-79-bandit-decide/route.ts` gravava o plano com `fromToken.toUpperCase()` — para `"cirBTC"` (grafia mixed-case, a mesma usada em `NETWORKS.arc.tokens`/`TOKEN_DECIMALS`/`COIN_IDS`/`TRADING_PAIRS`, todas em `lib/real-swap-executor.ts`/`lib/coin-ids.ts`) isso produz `"CIRBTC"`, uma chave que não existe em nenhuma dessas tabelas. Para `USDC`/`EURC`/`NATIVE` o bug nunca apareceu porque essas grafias já são naturalmente all-caps — `.toUpperCase()` era, sem querer, uma transformação sem propósito. Confirmado como lacuna pré-existente (o mesmo padrão já existia em `app/api/admin/cron-control/route.ts`, nunca exercitado porque nenhum plano manual usou `cirBTC`), não uma regressão do RI-BANK-81. Confirmado via Redis que nenhuma transação foi enviada — o gate roda antes de qualquer contato com KMS/provider/signer.

**Achado adicional, que mudou a direção da correção (RI-BANK-83):** tornar só o *gate* tolerante a caixa (sem corrigir a grafia armazenada) teria sido pior que o bug original — um plano com `"CIRBTC"` passaria a checagem, mas cairia silenciosamente em `TOKEN_DECIMALS["CIRBTC"] ?? 6`, usando **6 casas decimais em vez das 8 reais do cirBTC** (erro de 100x nos valores). Um bloqueio alto e explícito é mais seguro que deixar passar e quebrar silenciosamente mais adiante.

**Correção do RI-BANK-83 (parou de uppercasear na escrita) — deployada, mas incompleta:**
- `ri-bank-79-bandit-decide/route.ts`: parou de uppercasear — `chosen.fromToken`/`chosen.toToken` (já corretos, vindos de `ARC_BANDIT_PAIRS`) preservados como estão.
- Novo helper `resolveConfiguredTokenSymbol(tokens, input)` (`lib/real-swap-executor.ts`): resolve qualquer variação de caixa na entrada para a grafia exata configurada.
- Testado, documentado e deployado — mas ao pedir uma nova decisão do Bandit *depois* do deploy para confirmar na prática, o plano gravado ainda mostrou `fromToken: "CIRBTC"`, com o mesmo `materialFingerprint` de antes. A correção não teve efeito real nenhum.

**Por que RI-BANK-83 não resolveu (achado do RI-BANK-84):** existe uma terceira camada de uppercase que passou despercebida — `normalizePlanInput()` (`lib/cron-trading-state.ts`), chamada internamente por `store.savePlan()`, uppercasea `fromToken`/`toToken` **incondicionalmente para todo cron-plan**, não importa o que o chamador passe. É um invariante da camada de persistência (usada tanto pela rota manual quanto pela do Bandit), não uma escolha de nenhuma rota específica — o que RI-BANK-83 mudou nas duas rotas de escrita era, na prática, inócuo: `normalizePlanInput()` desfazia de qualquer jeito. Isso também significa algo mais importante: **todo cron-plan, sempre, desde que esse subsistema existe, tem `fromToken`/`toToken` em maiúsculas** — `USDC`/`EURC` só nunca expuseram isso por já serem naturalmente all-caps.

**Correção real (RI-BANK-84) — resolver na fronteira entre o plano (sempre maiúsculo) e o motor de execução (grafia nativa), não nas tabelas nem nas rotas de escrita:** `executeCronPlanWithKms()` (`lib/cron-trading-runtime.ts`) agora chama `resolveConfiguredTokenSymbol()` **uma vez**, logo depois de carregar a config de rede, transformando `plan.fromToken`/`plan.toToken` (sempre maiúsculos) na grafia nativa configurada (`"cirBTC"`, mixed-case) antes de repassar para `realSwap.executeSwap()`/`isStable()`. Nenhuma tabela de configuração (`NETWORKS.tokens`, `TOKEN_DECIMALS`, `COIN_IDS`, `PRICE_DIVIDERS`) foi alterada — continuam com a grafia mixed-case original, então o caminho separado do navegador (Pregão/`agentes-do-pregão.ts`, que nunca passa por `normalizePlanInput()`) fica completamente intocado. Um único ponto de tradução, não uma tolerância espalhada por dezenas de lookups num arquivo grande e sensível.

As mudanças do RI-BANK-83 nas duas rotas de escrita foram mantidas, mas os comentários foram corrigidos para não afirmarem mais que resolvem o problema — a checagem em `validatePlan()` (rota manual) ainda tem valor real (rejeita cedo, com tolerância a digitação humana, um símbolo genuinamente não configurado), só não determina mais a grafia final persistida.

**Outros pontos sensíveis a caixa no caminho de execução, verificados explicitamente:** `isStable()`/`STABLE_TOKENS` (sem bug — `cirBTC` nunca é membro do conjunto em nenhuma caixa); `COIN_IDS`/`PRICE_DIVIDERS`/`TOKEN_DECIMALS` (mesma família do bug original, agora alimentadas pela grafia resolvida); `lib/bandit-execution-feedback.ts`'s `convertTokenAmountToUsd()` (já normalizava internamente, seguro por construção). `lib/arc-direct-swap.ts`, `lib/agentes-do-pregão.ts` e `lib/agent-framework/coordinator.ts` pertencem a outros subsistemas nunca tocados pelo caminho de execução do cron/Bandit — fora do escopo.

**Teste de regressão:**
- `lib/security/ri-bank-83-token-case-fix.test.ts` — ainda válido como documentação da causa original: confirma que a tabela `NETWORKS.arc.tokens` em si aceita `"cirBTC"` e rejeita `"CIRBTC"` (comportamento das tabelas, não mudado), `resolveConfiguredTokenSymbol()` resolve variações de caixa, e a grafia errada teria mesmo caído no fallback perigoso de 6 decimais.
- `lib/security/ri-bank-84-cron-plan-case-boundary.test.ts` (novo) — reproduz o ciclo real e completo: um plano salvo via `MemoryCronTradingStateStore.savePlan()` com `fromToken: "cirBTC"` sai persistido como `"CIRBTC"` (confirma o invariante de `normalizePlanInput()`, não uma suposição); a resolução no limite de execução recupera `"cirBTC"` e alimenta `TOKEN_DECIMALS`/`COIN_IDS` com os valores reais (8 decimais, não o fallback de 6); o ciclo completo salvar→ler→resolver funciona de ponta a ponta; e o caminho do navegador (Pregão, `TRADING_PAIRS.arc`, lookup direto sem resolução) continua funcionando exatamente como antes, sem nenhuma mudança.

### RI-BANK-85/86 — preço de cirBTC vinha $0,0000 no preflight de saldo; causa era o padrão inteiro de self-call HTTP, não só `VERCEL_URL`

**Achado (RI-BANK-85):** com a correção de case (RI-BANK-84) funcionando e o faucet de cirBTC confirmado (0,0002 cirBTC real na signer), o disparo real ainda bloqueou por "saldo insuficiente: $0,0000" — o saldo em token estava certo, o preço usado para convertê-lo em USD é que vinha zero. Rastreado até `_getTokenPrice()` (`lib/real-swap-executor.ts`, RI-BANK-41): tokens voláteis sem preço resolvido caem, deliberadamente, em fallback **$0** (não $1,00 como stablecoins) — bloqueio conservador documentado, correto em si. A pergunta que sobrava era por que o preço nunca chegava a ser buscado de verdade.

**Tentativa de confirmação direta (RI-BANK-86) e uma complicação real:** a hipótese inicial (RI-BANK-85) era `VERCEL_URL` ausente/malformado quebrando `buildVercelInternalUrl()`, usada para montar a URL de self-call até a própria rota `/api/price`. Ao investigar mais a fundo antes de corrigir, encontrei uma contraevidência que complica essa hipótese isolada: **a mesmíssima função `buildVercelInternalUrl()` é usada, com sucesso, pelas chamadas de `/api/rpc-proxy`** (leitura de saldo) — confirmado por várias execuções reais bem-sucedidas ao longo desta sessão (`🔁 Balance via signer provider`, repetidamente, nos logs de produção). Se `VERCEL_URL` estivesse simplesmente ausente, o rpc-proxy também falharia sempre — e não falha. **Não obtive uma confirmação definitiva, com log direto, do exato ponto de falha** (o código não tinha nenhuma instrumentação de preço para inspecionar, e obter isso exigiria um novo deploy + disparo real, fora do meu alcance sem autorização) — e essa limitação é reportada aqui com a mesma honestidade pedida no ticket, não escondida atrás da correção.

**Correção — eliminar a classe inteira de fragilidade, não caçar a causa exata:** em vez de tentar diagnosticar precisamente por que o self-call falhava (resolução de URL, timeout ausente no `fetch()` original, ou alguma peculiaridade de function-to-function no Vercel), o caminho server-side de `_getTokenPrice()` (exatamente onde `executeCronPlanWithKms()` roda) parou de fazer *qualquer* chamada HTTP de volta pra própria aplicação. Agora chama `resolvePriceWithFallback(coinId)` (`lib/sosovalue-price-agent.ts`, novo) **em processo** — a mesma lógica de busca+fallback que `/api/price/route.ts` já usava, só que sem round-trip nenhum. A tabela `FALLBACK_PRICES`/`FALLBACK_CHANGE` (antes só na rota) foi movida pra lá, e a rota agora importa de lá em vez de duplicar. O caminho do navegador (client-side, `typeof window !== "undefined"`) continua usando `fetch()` relativo pra `/api/price` exatamente como antes — não mudou, e não precisava mudar. `buildVercelInternalUrl()` continua existindo e em uso para `/api/rpc-proxy` (RI-BANK-50/51), que não tem esse problema.

**Achado lateral, confirmado em teste:** `WBTC` e `cirBTC` compartilham o **mesmo** `coinId` em `lib/coin-ids.ts` (ambos mapeados pro preço do BTC) — descoberto ao escrever o teste de regressão, quando um cenário colidiu silenciosamente com o cache de outro. Documentado no teste; não é um bug (é intencional — os dois de fato seguem o preço do BTC), mas vale saber que os dois nunca terão preços independentes nesse sistema.

**Impacto retroativo — documentado com honestidade, sem alarmismo:** é provável (não confirmado com certeza total, pelas razões acima) que o preflight de saldo em USD de toda execução real via cron/KMS desta sessão — inclusive as bem-sucedidas em USDC/EURC (RI-BANK-71, RI-BANK-79, RI-BANK-81) — nunca tenha buscado um preço de verdade, e sim usado o fallback de stablecoin ($1,00) o tempo todo. Isso **nunca invalidou nenhum resultado real**: o dinheiro se moveu de verdade, on-chain, confirmado por `txHash` em cada caso; o preflight é só uma checagem de "há saldo suficiente pra tentar", não a fonte usada para decisão de lucro/trading (essa sempre veio de outro lugar — cotação real do pool, e desde RI-BANK-81, câmbio externo genuíno para o feedback do Bandit). Para USDC/EURC, o fallback de $1,00 está próximo o bastante do valor real que o efeito prático foi nulo. cirBTC foi o primeiro caso onde essa lacuna silenciosa produziu um resultado visivelmente errado ($0 em vez de ~$13,60), porque não é uma stablecoin.

**Teste de regressão:** `lib/security/ri-bank-86-server-price-no-vercel-url.test.ts` — confirma que o preço server-side resolve corretamente tanto com `VERCEL_URL` ausente quanto malformado (dois tokens distintos, evitando colisão de cache); confirma que uma falha da API externa (SoSoValue) cai no fallback real da tabela ($68.000 pro BTC/cirBTC), nunca no zero cego que bloqueou o trade real — e reproduz a conta exata do preflight (`0,0002 cirBTC × $68.000 = $13,60`, acima do mínimo de $0,095); e uma checagem estática confirmando que o caminho do navegador continua com `fetch()` relativo, intocado. `lib/security/ri-bank-38-server-price.test.ts` foi atualizado (o mecanismo que ele testava — self-call com URL absoluta — não existe mais no caminho server-side; agora confirma a chamada direta à API externa).

### RI-BANK-96 — sinal consultivo de ponte (`evaluateBridgeOpportunity`), sem automatização

**Contexto (RI-BANK-95):** o RI-BANK-94 validou a ponte CCTP USDC Base Sepolia → Arc Testnet ponta a ponta (approve/burn/attestation/mint) e destacou o que diferencia uma ponte de um swap: tempo de trânsito real de 10–30 min (vs. <1s do swap), com o capital exposto a mudança de preço enquanto está "no ar". O RI-BANK-95 propôs a Opção B: uma função **consultiva** que sinaliza quando "vale a pena considerar mover capital" — sem nunca disparar nada sozinha. O RI-BANK-96 implementa essa Opção B, exatamente como desenhada.

**Função (`lib/bandit-bridge-opportunity.ts`):** `evaluateBridgeOpportunity()` — função **pura** (sem rede, sem Redis, sem KMS) que recebe as condições atuais como input (`priceSourceUsd`, `priceDestinationUsd`, `cctpFeeUsd`, `gasSourceUsd`, `gasDestinationUsd`, janela/drift de trânsito, `tradeAmountUsd`) e devolve um sinal informativo: `bridges_worth_considering` ou `not_worth_considering`, com todos os números (`priceDifferenceUsd`, `barrierUsd`, `transitMarginUsd`, `totalBarrierUsd`, `surplusUsd`) — "sinal (0.01 > barreira 0.001) — vale considerar" ou o inverso. **Nunca gera, nunca autoriza, nunca se aproxima de um cron-plan** — não há nenhuma chamada a `savePlan()`/`executeCronPlanWithKms`. Helpers `computeBridgeFixedCostBarrier()` e `computeTransitMargin()` (exportados) para testes e composição.

**Barreira mínima (constante 3x, justificada):** `totalBarrier = (cctpFeeUsd + gasSourceUsd + gasDestinationUsd) × 3 + transitMarginUsd`, com `opportunity = priceDifference > totalBarrier` (estrito). A constante **3x é de custo fixo** (1x paga o custo + 2x cobrem erro de estimativa de gas nas duas pernas e descolamento adverso da referência antes da 1ª perna), **não é o 10x do slippage** (`LIQUIDITY_DEPTH_MULTIPLIER`, lib/route-verifier.ts) — que é multiplicador de **profundidade de pool** (protege a execução de um swap de deslocar o preço). São duas naturezas de barreira diferentes; o 3x está documentado e coberto por teste. A **margem de trânsito** é separada e proporcional: `valor movido × drift por hora × janela de horas` (default: 0,5h × 0,1%/h × USDC ~$2 = $0,001) — cobre o risco RI-BANK-95 R1/R2 (hedge de preço no tempo em trânsito).

**Riscos de execução parcial (documentados, não resolvidos nesta etapa):** se um dia esta função for conectada a execução real, o risco R7 de RI-BANK-95 volta ao centro: burn confirmado (origem debitada) + mint falhado (destino não crédito) — capital preso no limbo; exige caminho de recovery (reAttest + mint manual, validado em RI-BANK-94) e 1 ponte simultânea máxima. Este ticket **não** resolve isso: a rota é read-only e a função é consultiva; qualquer passo de execução futura (Fase 2) precisa re-decidir essa exposição.

**Endpoint (`GET /api/internal/ri-bank-96-bridge-opportunity`):** mesmo padrão de diagnóstico das rotas de verificação (bearer `ADMIN_PANIC_KEY`, read-only). Parâmetros de query opcionais: `pair`, `source`, `dest`, `priceSourceUsd`, `priceDestinationUsd`, `cctpFeeUsd`, `gasSourceUsd`, `gasDestinationUsd`, `transitHours`, `driftPerHour`, `amountUsd` — com defaults calibrados nos dados reais de RI-BANK-94 (fee CCTP 0,00026; gas testnet ~0). Resposta: `{ ok, readOnly, note, input, result, timestamp }`. Não conectado ao Bandit nem ao cron (RI-BANK-96).
...
- **Diagnóstico count:0 (copia5, 07/08):** runs do GitHub reportavam "success" mesmo sem gravar nada — o `curl` do workflow não usa `--fail`, então HTTP 404 da rota (deploy stale sem as rotas) retorna exit 0 → GitHub marca verde. Runs #1–3 (19:06/19:38/20:22Z) todos bateram no deploy antigo. Corrigido com deploy forçado (`npx vercel --prod`) — desde então, run #4 (21:11Z, `schedule`) gravou observação no Redis.
- **Round-trip unificado (copia6, 07/08):** collect e export já usavam a mesma conexão (`lib/kv.ts` `getRedis()`/`isKvConfigured()`, env `KV_REST_API_URL/TOKEN`) e a mesma chave `riBank99ObservationsKvKey()`. Adicionado teste de regressão `lib/security/ri-bank-99-collect-export-roundtrip.test.ts` (escreve via `appendObservation` → lê via `readObservations`, mesmo config; roda com `npx tsx` + `.env.local`, namespace `local`, backup/restore da chave). Rodar com `DOTENV_CONFIG_PATH=.env.local` para carregar o `.env.local`.
