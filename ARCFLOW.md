...
**Correção escolhida — resolver para a grafia canônica na escrita, não afrouxar o gate de execução:**
- `ri-bank-79-bandit-decide/route.ts`: parou de uppercasear — `chosen.fromToken`/`chosen.toToken` (já corretos, vindos de `ARC_BANDIT_PAIRS`) são preservados como estão.
- Novo helper `resolveConfiguredTokenSymbol(tokens, input)` (`lib/real-swap-executor.ts`): resolve qualquer variação de caixa na entrada para a grafia exata configurada, sem nunca inventar uma forma diferente. Usado em `app/api/admin/cron-control/route.ts`'s `validatePlan()` — essa rota é acionada manualmente (curl/painel), então mantém tolerância a digitação humana (`"usdc"`, `"Usdc"`, `"CIRBTC"` todos resolvem para a grafia certa), mas agora sempre armazena a forma canônica, nunca uma que não bate com `TOKEN_DECIMALS`/`COIN_IDS`/`PRICE_DIVIDERS`.
- O gate em `lib/cron-trading-runtime.ts` (`executeCronPlanWithKms()`) permanece **estrito** (comparação exata) — decisão deliberada: como os dois pontos de escrita agora garantem grafia canônica, o gate nunca precisa ser tolerante; mantê-lo estrito preserva a falha alta e explícita como última linha de defesa, em vez de arriscar deixar passar uma grafia errada para quebrar silenciosamente mais adiante.

**Outros pontos sensíveis a caixa no caminho de execução, verificados explicitamente:** `isStable()`/`STABLE_TOKENS` (não tem bug — `cirBTC` nunca é membro do conjunto em nenhuma caixa, resposta correta de qualquer forma); `COIN_IDS`/`PRICE_DIVIDERS`/`TOKEN_DECIMALS` (mesma família do bug original, todas indexadas pela mesma grafia `cirBTC`, agora corretamente alimentadas); `lib/bandit-execution-feedback.ts`'s `convertTokenAmountToUsd()` (já normalizava internamente com `.toUpperCase()` antes de comparar contra literais `"USDC"`/`"EURC"` — seguro por construção). `lib/arc-direct-swap.ts`, `lib/agentes-do-pregão.ts` e `lib/agent-framework/coordinator.ts` têm padrões próprios de normalização de caixa, mas pertencem a outros subsistemas (AMM legado, Pregão client-side) nunca tocados pelo caminho de execução do cron/Bandit — fora do escopo desta correção.

**Teste de regressão:** `lib/security/ri-bank-83-token-case-fix.test.ts` — reproduz o cenário exato do diagnóstico: um plano com `fromToken: "cirBTC"` passa o gate (documentando a correção), a forma antiga `"CIRBTC"` continua reprovando (documentando o bug, não só a correção); `resolveConfiguredTokenSymbol()` resolve qualquer variação de caixa (`cirBTC`/`CIRBTC`/`cirbtc`/`CirBtc` → `cirBTC`; `usdc`/`Usdc` → `USDC`) e devolve `undefined` para um símbolo inexistente; confirma que a grafia errada teria mesmo caído no fallback perigoso de 6 decimais; confirma `isStable()` correto para as três grafias canônicas; e uma checagem estática confirmando que a rota do Bandit não uppercasea mais.
